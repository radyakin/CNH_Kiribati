...
write.csv(fisheriesTidy_903, "Outputs/fisheriesTidy_903.csv", row.names = FALSE)
...
write.csv(other_answers, "Outputs/fisheries_903R_other_answers.csv", row.names = FALSE)
...
write.csv(fisheriesTidy_922, "Outputs/fisheriesTidy_922.csv", row.names = FALSE)
...
write.csv(market_survey_no_weight, file = file.path(outdir, "market_survey_no_weight.csv"), row.names = FALSE) # n = 1

# market_survey_price <- plotDF_food %>% group_by(interview__id, roster__id, price, weight) %>% mutate(n=n()) %>% arrange(desc(n)) %>% filter(row_number()==1) %>% ungroup() %>% select(-n)# keep top row of each grouping
# Inspect and standardize units: 
# market_survey_price %>% select(unit_roster__id, unit_other) %>% unique() %>% print(n=40) 
...
market_survey_price <- marketSurvey %>% 
  ...
  mutate(weight = if_else(unit_roster__id == "(kg) Kilograms", true = weight * 1000, false = weight),
         unit_roster__id = if_else(unit_roster__id == "(kg) Kilogram", true = "(g) Grams", false = unit_roster__id)) %>%
...
# Mass: (g) Grams, grams(g), (lbs Pounds, Pounds (lb))
