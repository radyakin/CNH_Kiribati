...
6) Is there a way to look at how many "complete surveys" have been done by checking some of the critical info and seeing whether it has been finished or not? This will help us to have a total sample size finished so far to then calculate how many NAs in a given other table are strange.
- Yes, but we will just have to define what a "complete survey" would look like

ii. Missing age and sex as I mentioned is very concerning because that should have been collected mandatorily for every interview. Could you check to see where this is blank N/A for age and sex and where there is no information in the interview or if there is info there. I really need to figure this out. Very strange.

There are `r sum(is.na(data_clean$L1_q3_gender))` NAs in the gender column of the raw data (after the deleted = TRUE rows have been deleted). Is there any chance that more of the rows should be deleted? The rows with NA in gender tend to have many NAs in them.  

There are `r sum(is.na(data_clean$L1_q3_gender))` NA's in the gender column.

We can go through and calculate this for any column or combo of colunms. I am just not sure how many NAs we expect. Let's discuss this in person. 
...
The percent of breastfeeding women between the ages of 12 and 40 is `r round(100*(n.breastfeeding/n.F12to40))`%. 

Double check breastfeeding and lactating... I think I made a mistake because lactating should only refer to women (so age/sex cats I specified) and the breastfeeding should only refer to children who are doing that. But, given the numbers, something makes me think the question is being asked incorrectly, or the classification is being done strangely... any thoughts?
- I am not sure if I understand this correctly, so I haven't changed it yet. I can show you when we meet in person (this will be easy to straighten out). 
